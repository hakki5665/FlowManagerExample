...
public class FileFlowService implements IFileFlowService {
    ...
    public UploadFileResponse initFlow(MultipartFile file) {
        ...
        saveTaskAndOutboxInTransaction(taskId, storagePath);
    ...
    @Transactional
    @SneakyThrows
    public void saveTaskAndOutboxInTransaction(UUID taskId, String storagePath) {
        ...
        ToConvertEvent event = new ToConvertEvent(taskId, storagePath);
    ...
    public void completeFlow(ConvertedEvent event) {
        FileTask task = taskRepository.findById(event.requestId()).orElse(null);

        if (task == null) {
            log.warn("Получен ответ для несуществующей задачи с ID: {}", event.requestId());
            return;
        }
