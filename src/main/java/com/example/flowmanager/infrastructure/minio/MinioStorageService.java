...
public class MinioStorageService implements IStorageService {
    ...
    public InputStream download(String fullObjectPath) {
        if (fullObjectPath == null || !fullObjectPath.contains("/")) {
            throw new IllegalArgumentException("Неверный формат пути к объекту в хранилище");
        }

        String bucket = fullObjectPath.substring(0, fullObjectPath.indexOf("/"));
